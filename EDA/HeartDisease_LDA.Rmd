...
```
The approximate error rate for classification is highest at $42\%$ between blood pressure and age, and lowest at $28.5\%$ between maximum heart rate and cholesterol. This gives a range of classification error between $28.5%-42%$ between the numerical variables of the Bayesian classification, slightly better than that of the linear discriminant analysis.

Overall, the linear discriminant model is better for detecting true positives, whereas the Bayesian classification is better for detecting true negatives. The balanced accuracy of the linear discriminant model is $84.86\%$, whereas the balanced accuracy of the Bayesian classification is $66.13\%$. The linear discriminant model should be used over the Bayesian classification, as it is more important to detect heart disease that is present than it is to clear someone of having it when they do not.
