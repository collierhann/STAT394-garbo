...
set.seed(1234567890, kind = "Mersenne-Twister")
library(readr)
HDdataone <- read_csv("Heart_Disease_Prediction.csv", col_types = cols(Sex = col_factor(levels = c("Female", "Male")), Heart.Disease = col_factor(levels = c("Absence", "Presence"))))

HDdataone$Interaction <- interaction(HDdata$Sex, HDdata$Heart.Disease)

(l <- levels(HDdataone$Interaction))

levels(HDdataone$Interaction) <- list("F"=l[1], "M"=l[2], "f"=l[3], "m"=l[4])

HD_ind <- sample(c("Tr", "Te"), size = nrow(HDdata), replace = TRUE, prob = c(.6, .4))
#Explicitly split the data in "Train" and "test"
HD_Train <- HDdataone[HD_ind=="Tr", -c(1,3,15)]
HD_Test <- HDdataone[HD_ind=="Te",-c(1, 3,15)]
(LDA1 <- lda(Interaction ~ Age+HD_Train$`Chest pain type`+BP+Cholesterol+HD_Train$`Max HR`, HD_Train))

pred1 <- predict(LDA1)
ldahist(data = pred1$x[,1], g=Train$Interaction)
ggord(LDA1, Train$Heart.Disease, alpha=.5, xlims=c(-5,5), ylim = c(-5, 5))

library(psych)
library(klaR)
partimat(Interaction ~ Age+HD_Train$`Chest pain type`+BP+Cholesterol+HD_Train$`Max HR`, data=HD_Train, method="lda")
...
realisticpredicted1 <- predict(LDA1, HD_Test)$class
(RCM1 <- table(realisticpredicted1, Actual=HD_Test$Interaction))
sum(diag(RCM1))/sum(RCM1)
```
